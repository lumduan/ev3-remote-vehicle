...
| **The LCD can only be drawn by `ev3dev2.display`, and only from Brickman.** `print()` reaches the LCD because Brickman launches programs on a `conrun` console, but that console is 44x21 in a 4x6 font. Writing `/dev/fb0` loses to fbcon, and the documented `chvt` fix, plus unbinding fbcon, are both root-only while `sudo` here wants a password | Measured 2026-09-01: printed through `conrun` and read back from `/dev/vcs5`; console geometry from `/dev/vcsa5`. `agent/tank_drive.py` draws with `ev3dev2`, confirmed visible from the File Browser by the operator |
| **Writing to an evdev node injects events.** `evdev_write` calls `input_inject_event`, so an event written to a device's node arrives as if the hardware had produced it. A press and release written to the brick's button node read back out of it, and **Brickman redrew its menu in response: 445 blocks of framebuffer changed, against 0 for a screen left alone for the same interval** | Written and read back from ssh, 2026-09-02. This is what makes `agent/pad_buttons.py` possible, and it needs no `uinput` device and no module |
| **Every `/dev/input/event*` is writable by `robot`** - `root:input` 0660, and `robot` is in `input`. `/dev/uinput` exists on the same terms, unused | `ls -l`, 2026-09-02. Nothing here needs root |
| **The brick's buttons are `event1`**, `Phys=gpio-keys/input0`, and declare **`B: KEY=1680 0 0 10004000`** = codes **14, 28, 103, 105, 106, 108**: `KEY_BACKSPACE`, `KEY_ENTER`, `KEY_UP`, `KEY_LEFT`, `KEY_RIGHT`, `KEY_DOWN`. Six buttons, and Back is one of them | `/proc/bus/input/devices`, 2026-09-02, decoded. A fixture in this repository had this mask ending `10000000`, which dropped bit 14 and made the brick look as though it had no Back button; that fixture was invented rather than read, and is now corrected |
| **The brick's speaker is an input device too.** `LEGO MINDSTORMS EV3 Speaker` on `event0`, `B: EV=40001` carrying bit 18 `EV_SND`, `B: SND=6` carrying `SND_BELL` and `SND_TONE`. Writing `EV_SND`/`SND_TONE` with a frequency starts a tone and writing 0 stops it | Sounded on the brick, 2026-09-02. No `aplay`, no `beep` binary, and no third-party import |
| **Cross is 304 and Share is 314** on the DualShock 4 | Each held down and read back with `EVIOCGKEY`, 2026-09-02, the same way L1 gave 310 and R1 gave 311 |
| **Opening the pad's event node races udev.** Pressing PS gives `EACCES` on `/dev/input/event4` for the first few seconds, because udev creates the node before applying the rule that puts it in the `input` group | Verified 2026-09-02. Two failures two seconds apart, then success on the third look. The reconnect loop is unbounded and polls every 2 s, so it heals itself; bounding it would break this |
| **Autostart works.** After `sudo loginctl enable-linger robot`, the user service starts `pad_buttons.py` at boot with nobody logged in and no computer attached | Verified on hardware 2026-09-02. Fresh boot, `Linger=yes`, `active (running)`, main pid under `user@1000.service` |
| **The buttons are not live for roughly 30 seconds after boot.** systemd started the unit at 06:41:14; the program's first line appeared at 06:41:42 | Verified 2026-09-02. That is Python interpreter startup on a 300 MHz ARM9, not a fault. Pressing PS before then looks like a failure and is not |
| **ConnMan restores the Bluetooth radio across a reboot** without intervention: `Powered = True` on a fresh boot | Verified 2026-09-02, the boot after the one where it was first switched on. Previously believed from persisted state only |
| **Under the service, the program's output goes to the journal, not `pad_buttons.log`.** The log file is only written on the daemonised path, where `daemonise()` redirects the streams | Verified 2026-09-02 - the log still held entries from before the reboot while `systemctl --user status` showed the live ones |
| **There is no root-free way to start a program at boot on this brick.** No cron at all (`crontab` and `cron` absent, service inactive), no `/etc/rc.local`, no `/etc/xdg/autostart`, `/etc/systemd/system` root-only. The systemd **user** instance does run and `/run/user/1000` exists, but `Linger=no`, so user services wait for a login that never happens at boot | Checked 2026-09-02. `sudo loginctl enable-linger robot` is the least-privilege fix: one root command ever, after which the unit, enabling and editing are all owned by `robot` |
| **`/home` is `root:root drwxr-xr-x`**, so a folder directly under it needs root. `/home/robot/` does not | `ls -ld`, 2026-09-02. Why `pad_buttons` lives at `/home/robot/pad_buttons/` |
| **ConnMan remembers the Bluetooth radio across reboots.** `connmanctl technologies` reports `Powered = True`, persisted from when it was first enabled | `connmanctl`, 2026-09-02. The radio should therefore be up at boot without intervention - one reboot away from verified |
